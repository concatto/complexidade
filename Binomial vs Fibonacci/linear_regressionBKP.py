...
def linear_regression(xs,ys,label):
    # will be a log-log regression line.
    lxs = np.log(xs)
    lys = np.log(ys)
    ...
    legenda1 = mlines.Line2D([], [], color='blue',markersize=15, label="Binomial")
    legenda2 = mlines.Line2D([], [], color='orange',markersize=15, label="Fibonacci")
    plt.legend(handles=[legenda1, legenda2])
